...
``` perl Example context/assignment combinations
...
``` perl `each` operator in a `while` loop
...
``` perl Incorrect quick and dirty parameter checking
...
``` perl OK quick and dirty parameter checking
